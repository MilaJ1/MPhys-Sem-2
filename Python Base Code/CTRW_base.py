...
def gauss_step_3d_bounded(xyz_array, diff_coeff, diff_time,r_nuc=3000):
    ...
    while distance_centre > r_nuc:
        ...
        distance_centre = get_distance_3d(new_coords,np.array([0,0,0]))
...
def ctrw_3d_interaction(initial_pos, diff_coeff, diff_time, run_time, min_wait_time, anom_diff_exp,
    ...
    while (time1 < run_time and time2 < run_time) and repair==0:
        ...
        if time1 < time2:
            ...
            times1 = np.append(times1, time1)
            coords1 = gauss_step_3d_bounded(coords1, diff_coeff, diff_time)
        else:
            waiting_time2 = get_waiting_time(min_wait_time, anom_diff_exp)
            time2 += waiting_time2
            times2 = np.append(times2, time2)
            times2 = np.append(times2, time2)
            coords2 = gauss_step_3d_bounded(coords2, diff_coeff, diff_time)
